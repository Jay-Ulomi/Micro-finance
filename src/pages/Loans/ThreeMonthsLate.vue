      ...
      <div class="flex justify-between items-start">
        <div>
          <div class="flex items-center space-x-2">
            <h1 class="text-2xl font-bold text-gray-900">3 Months Late Loans</h1>
            <a href="#" class="text-sm text-primary-600 hover:text-primary-800">Help</a>
          </div>
          <p class="text-gray-600 mt-2">
            This shows loans overdue by 90 days or more. This is calculated from the loan schedule
            for loans that are Missed Repayment, Arrears, or Past Maturity. We look at the last loan
            schedule date where the balance was 0 and then calculate the number of days between that
            schedule date and today.
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      <div v-if="showAdvancedSearch" class="p-6">
        <div class="space-y-4">
          <!-- Overdue By Filter -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Overdue By</label>
            <div class="flex items-center space-x-4">
              <select v-model="filters.overdueBy" class="form-input w-32">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex items-center justify-between">
          <h3 class="text-lg font-medium text-gray-900">3 Months Late Loans</h3>
          <div class="flex items-center space-x-2">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-else-if="filteredLoans.length === 0">
              <td colspan="11" class="px-6 py-12 text-center">
                <div class="text-center">
                  <p class="text-gray-500">No data found. No loans found.</p>
                </div>
              </td>
            </tr>
            <tr
            ...
            >
              ...
              <!-- Balance -->
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                <span class="font-semibold text-red-600">{{ formatCurrency(loan.balance) }}</span>
              </td>
              <!-- PendingDue -->
...
// Mock data for 3 months late loans (currently empty as shown in UI)
const mockLoans = ref([]);
...
const filteredLoans = computed(() => {
  ...
  // Apply overdue filter (90+ days)
  loans = loans.filter((loan) => loan.daysOverdue >= filters.value.overdueBy);
...
const searchLoans = async () => {
  ...
  try {
    ...
    console.log("Searching 3 months late loans with filters:", filters.value);
  } catch (error) {
...
const getStatusClass = (status: string) => {
  const statusClasses: { [key: string]: string } = {
    ...
    past_maturity: "bg-red-100 text-red-800",
